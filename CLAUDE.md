...
python finetune.py --model-name MobileCLIP2-S0 --root-dir ./data ...
```
...
Generates fine-grained retrieval training data from images via an OpenRouter VLM API. Config and prompts are in `DEFINE.py`; the builder script is `openrouter_vlm_dataset_builder.py`.
...
- The tokenizer is always `open_clip.get_tokenizer("ViT-B-32")` regardless of which MobileCLIP2 variant is used.
