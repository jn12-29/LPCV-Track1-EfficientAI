...
```bash
curl -L -o model.gguf https://huggingface.co/ggml-org/gpt-oss-20b-GGUF/resolve/main/gpt-oss-20b-mxfp4.gguf
```
...
#### Run on CPU
```bash
GGML_HEXAGON_NDEV=0 llama-completion --model model.gguf --n-predict -1 --ctx-size 128 --system-prompt "You are a helpful assistant. Be helpful but brief." --prompt "Write a short poem about the stars and the vastness of space." --seed 1 --single-turn --no-display-prompt --n-gpu-layers 0
```
#### Run on GPU
```bash
GGML_HEXAGON_NDEV=0 llama-completion --model model.gguf --n-predict -1 --ctx-size 128 --system-prompt "You are a helpful assistant. Be helpful but brief." --prompt "Write a short poem about the stars and the vastness of space." --seed 1 --single-turn --no-display-prompt -fa off
```
#### Run on HTP (NPU)
```bash
GGML_HEXAGON_NDEV=4 llama-completion --model model.gguf --n-predict -1 --ctx-size 128 --system-prompt "You are a helpful assistant. Be helpful but brief." --prompt "Write a short poem about the stars and the vastness of space." --seed 1 --single-turn --no-display-prompt --no-mmap -t 6 --cpu-mask 0xfc --cpu-strict 1 -ctk q8_0 -ctv q8_0 -fa on --batch-size 128 --device "HTP0,HTP1,HTP2,HTP3"
```
