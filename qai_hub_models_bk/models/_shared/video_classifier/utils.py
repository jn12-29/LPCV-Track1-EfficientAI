...
def normalize_base(
    ...
    """
    Normalize video using mean and standard deviation.
    ...
    video
        Input video torch.Tensor.
    mean
        Mean to be subtracted per channel of the input.
    std
        Standard deviation to be divided per each channel.
...
def read_video_per_second(path: str) -> torch.Tensor:
    """
    Read video from path and convert to torch tensor.
...
def preprocess_video_224(input_video: torch.Tensor) -> torch.Tensor:
    """
    Preprocess the input video correctly for video classification inference.
