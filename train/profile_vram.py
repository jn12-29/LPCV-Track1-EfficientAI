...
Usage:
    python train/profile_vram.py --model-name MobileCLIP2-S2 \
...
def make_fake_batch(batch_size: int, num_hard_negatives: int, device: torch.device):
    ...
    if num_hard_negatives > 0:
        neg_tokens = torch.randint(0, 49408, (batch_size, num_hard_negatives, 77), device=device)
...
def measure_one(
    ...
    images, pos_tokens, neg_tokens, neg_mask = make_fake_batch(batch_size, num_hard_negatives, device)
    ...
    with autocast(device_type=device.type, enabled=use_amp):
        ...
        if loss_type == "siglip":
            ...
            if num_hard_negatives > 0:
                ...
                flat = neg_tokens.view(-1, 77)
                neg_feats = model.encode_text(flat)
                neg_n = F.normalize(neg_feats.view(batch_size, num_hard_negatives, -1), dim=-1)
...
def main():
    ...
    parser.add_argument(
        "--batch-sizes", default="32,64,96,128,192,256,320,384,512",
    ...
    print(f"{'BatchSize':>10} {'PeakVRAM(MB)':>14} {'PeakVRAM(GB)':>14} {'Util%':>8} {'Status':>8}")
    print("-" * 60)
    ...
    for bs in batch_sizes:
        try:
            peak_mb = measure_one(
                model, loss_fn, optimizer, bs,
                args.num_hard_negatives, args.loss_type, device,
                use_amp=args.amp,
            )
            util_pct = peak_mb / total_mb * 100
            status = "OOM" if peak_mb > total_mb else "OK"
            print(f"{bs:>10} {peak_mb:>14.0f} {peak_mb/1024:>14.2f} {util_pct:>8.1f} {status:>8}")
